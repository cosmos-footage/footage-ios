...
- Next step is to extract remaining `SceneDelegate` side effects, starting with foreground route dispatch and scene lifecycle helper placement cleanup, still keeping the default launch on `Main` until parity QA is complete.
